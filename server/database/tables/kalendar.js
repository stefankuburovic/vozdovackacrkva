...
app.post('/api/kalendar', (req, res) => {
    const { praznik, stari, novi, post, ime_dana, mesec } = req.body;

    con.connect(function(err) {
        if (err) throw err;
        var sql = `INSERT INTO kalendar (praznik, stari, novi, post, ime_dana, mesec) VALUES (?, ?, ?, ?, ?, ?)`;
        con.query(sql, [praznik, stari, novi, post, ime_dana, mesec], function (err, result) {
            if (err) throw err;
